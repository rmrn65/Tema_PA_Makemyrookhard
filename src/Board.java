...
public class Board {
    ...
    public Board(Board board) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++)
                this.object_matrix[i][j] = board.object_matrix[i][j];
        }
        ...
        if (squares.length() == 5) {
            ...
            switch (squares.charAt(4)) {
                case 'q':
                    ...
                    break;
                case 'r':
                    object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Rook(promotedPawn.current_position, promotedPawn.color);
                    break;
                case 'b':
                    object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Bishop(promotedPawn.current_position, promotedPawn.color);
                    break;
                case 'n':
                    object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Night(promotedPawn.current_position, promotedPawn.color);
                    break;
                default:
                    break;
            }
    ...
    public ArrayList<String> getAllLegalMoves() {
        ...
        if (colorToMove.compareTo("black") == 0)
            pieceSet = new ArrayList<>(BlackPieces);
        else
            pieceSet = new ArrayList<>(WhitePieces);
        for (Piece p : pieceSet) {
            if (!(p instanceof King)) {
                if (p.canMove(this) && this.k.canIMove(p.current_position, this)) {
                    legalMoves.addAll(p.generateMoves(this));
                }
            }
            // TODO KING
        }
