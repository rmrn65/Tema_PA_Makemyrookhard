...
public class Board {
    ...
    public void move(String squares){
        ...
        if(squares.length() == 5) {
            ...
            switch (squares.charAt(4)) {
                case 'q': object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Queen(promotedPawn.current_position, promotedPawn.color);
                        break;
                case 'r': object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Rook(promotedPawn.current_position, promotedPawn.color);
                        break;
                case 'b': object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Bishop(promotedPawn.current_position, promotedPawn.color);
                        break;
                case 'n': object_matrix[promotedPawn.current_position.charAt(1) - '1'][promotedPawn.current_position.charAt(0) - 'a'] = new Night(promotedPawn.current_position, promotedPawn.color);
                        break;
                default:
                        break;
            }
    ...
    public static double Evaluate(Board board,String myColor){
        ...
        double individualPiecesCost=myPieces.stream().MapToDouble(piece -> IndividualCost(board,piece)).sum()-enemyPieces.stream().MapToDouble(piece ->  IndividualCost(board,piece)).sum();
    }

    double IndividualCost(Board board,Piece myPiece){
        int posX,posY;
        ...
        for(i=posX-1;i>0;i--){
            if(Helper.computeIndividual(i,posY)
                break;
        }
        for(i=posX+1;i<=8;i++){
            if(Helper.computeIndividual(i,posY))
        ...
        for(i=posY-1;i>0;i--){
            if(Helper.computeIndividual(posX,i))
                break;
        }
        for(i=posY+1;i<=8;i++){
            if(Helper.computeIndividual(posX,i))
        ...
        for(i=poX-1,j=posY-1;i>0 && j>0;i--,j--){
            if(Helper.computeIndividual(i,j))
                break;
        }
        for(i=poX-1,j=posY+1;i>0 && j<=8;i--,j++){
            if(Helper.computeIndividual(i,j))
                break;
        }
        for(i=poX+1,j=posY-1;i<=8 && j>0;i++,j--){
            if(Helper.computeIndividual(i,j))
                break;
        }
        for(i=poX+1,j=posY+1;i<=8 && j<=8;i++,j++){
            if(Helper.computeIndividual(i,j))
                break;
        }

        private static class Helper{
            Boolean reachedPiece=false,stop=false;
            Boolean static computeIndividual(int x,int y,Function movesThisWay){
